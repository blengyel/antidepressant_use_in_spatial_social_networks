...
# This script generates the cross-sectional co-efficient plot, 
# the interplot visualizations between SD and LC,
# and the dynamic co-efficient plot.
...
placebo_logit<- glm(z1 ~ z_H_norm + z_cc_in_ERnorm + z_ln_d 
                    + z_n50 + m1 
                    + z_age + z_ln_income_pop 
                    + z_unemp_pop 
                    + as.character(county_id), 
                    data = d, family="binomial")
summary(placebo_logit)
...
#t=coeftest(placebo_logit, vcov = vcovHC(placebo_logit, "HC1"))
t=summary(placebo_logit)$coefficients
var1_p <- data.frame(Variable = "Spatial Diversity",
                     Coefficient = t[2,1],
                     SE = t[2,2])
var2_p <- data.frame(Variable = "Local Cohesion",
                     Coefficient = t[3,1],
                     SE = t[3,2])
var3_p <- data.frame(Variable = "Degree",
                     Coefficient = t[4,1],
                     SE = t[4,2])
var4_p <- data.frame(Variable = "Share of friends within 50km",
                     Coefficient = t[5,1],
                     SE = t[5,2])
# var5_p <- data.frame(Variable = "Male",
#                      Coefficient = t[6,1],
#                      SE = t[6,2])
# var6_p <- data.frame(Variable = "Age",
#                      Coefficient = t[7,1],
#                      SE = t[7,2])
# var7_p <- data.frame(Variable = "Average income in hometown",
#                      Coefficient = t[8,1],
#                      SE = t[8,2])
# var8_p <- data.frame(Variable = "Unemplyment rate in hometown",
#                      Coefficient = t[9,1],
#                      SE = t[9,2])

allModelFrame <- data.frame(rbind(
  var1_m, var2_m, var3_m, var4_m,
  var1_p, var2_p, var3_p, var4_p))
